...
import SpecificTestStepsMaster from './SpecificTestStepsMasterOld.js';
import TestCaseResultsMaster from './TestCaseResultsMaster';
...
interface State {
    ...
    activeTab: 'details' | 'specific' | 'results';
    isModalOpen: boolean;
...
export default function TestStepsDetailsMaster(props: Props) {
    ...
    useEffect(() => {
        const init = async () => {
            ...
            await getExecutionComponentsByObjectId(props.CurrAddEditDetails.ObjectId || "");
    ...
    const handleSequenceRefreshConfirm = async (): Promise<void> => {
        ...
        try {
            ...
            const resp: any = await apiRequest("/RefreshTestStepsSequence", {
    ...
    // Component Type Change Handler - Cloned from ComponentKeysMaster
    ...
    // Component ID Change Handler - Cloned from ComponentKeysMaster
    ...
    const getExecutionComponentsByObjectId = async (ObjectId: string = ""): Promise<void> => {
        try {
            if (!ObjectId) {
                setState({ ExecutionComponentList: [] });
                return;
            }

            const resp: any = await apiRequest("/GetExecutionComponentByObjectId", {
                ObjectId: ObjectId
            });

            if (resp.ResponseData && resp.ResponseData.length > 0) {
                const formattedComponents: Option[] = resp.ResponseData.map((comp: any) => ({
                    value: comp.ExecutionComponentId || "",
                    label: `${comp.ExecutionComponentId} - ${comp.Component || ""}` || comp.ExecutionComponentId
                }));
                setState({ ExecutionComponentList: formattedComponents });
            } else {
                setState({ ExecutionComponentList: [] });
            }
        } catch (err) {
    ...
    const handleModalSave = async (): Promise<void> => {
        ...
        try {
            ...
            await apiRequest("/AddUpdateTestStepsDetails", itemToSave as any);
    ...
    // First confirmation - ask to delete
    const handleDeleteClick = (item: TestStepDetail): void => {
        setState({
            itemToDelete: item,
    ...
    // After first confirmation - ask about sequential update
    const handleDeleteConfirm = (): void => {
        // Close first modal and show second confirmation
        setState({
            showDeleteConfirm: false,
        });
        
        // Show second confirmation about updating step numbers
        if (window.confirm("Do you want to update the remaining step numbers sequentially?")) {
            setState({ shouldUpdateStepNos: true });
        } else {
            setState({ shouldUpdateStepNos: false });
        }
    ...
    const performDelete = async (): Promise<void> => {
        ...
        try {
            if (state.itemToDelete.TestStepDetailId) {
                ...
                const deleteRequest = {
                    ...state.itemToDelete,
                    updateSequential: state.shouldUpdateStepNos
                };

                const resp: any = await apiRequest("/DeleteTestStepsDetails", deleteRequest);
    ...
    const handleDeleteItem = async (item: TestStepDetail): Promise<void> => {
        ...
        // This will be called after ConfirmPopup confirmation
        performDeleteWithSequentialCheck(item);
    };
    ...
    const handleTabChange = (tab: 'details' | 'specific' | 'results'): void => {
    ...
    return (
        <div className="pt-0 pb-6">
            ...
            {state.activeTab === 'specific' && (
                <SpecificTestStepsMaster TestStepsId={props.CurrAddEditDetails.TestStepsId} />
            )}

            {state.activeTab === 'results' && (
                <TestCaseResultsMaster TestStepsId={props.CurrAddEditDetails.TestStepsId} />
            )}
            ...
            >
                {state.currentEditItem && (
                    <div className="space-y-4">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            {/* Component - Now uses the cloned functionality */}
                            <div>
                                <label className="block text-sm font-medium text-gray-700 mb-1">
                                    Component
                                </label>
                            ...
                            {/* Description - Now auto-populated */}
                            <div>
                                <label className="block text-sm font-medium text-gray-700 mb-1">
                                    Description
                                </label>
