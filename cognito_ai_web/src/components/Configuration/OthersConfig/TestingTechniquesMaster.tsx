...
interface TestingTechniqueItem {
...
interface GetTestingTechniquesResponse {
  ResponseData: TestingTechniqueItem[];
  TotalRecords: number;
}
...
type ValidateFieldKeys = 'TestingTechnique';
...
interface State {
  ...
  CurrentPage: number;
  TotalRecords: number;
  TestingTechniquesMaster: TestingTechniqueItem[];
  ...
  CurrAddEditObj: TestingTechniqueItem;
...
export default function TestingTechniquesMaster(_: Props) {
  ...
  const getData = async (SearchQuery = '', PageNo = 1): Promise<void> => {
    try {
      const resp = (await apiRequest('/GetTestingTechniquesMasterPaginationFilterSearch', {
        PageNo,
        StartDate: '',
        EndDate: '',
        SearchString: SearchQuery,
      })) as GetTestingTechniquesResponse;

      if (resp.ResponseData.length > 0) {
        setState({ TestingTechniquesMaster: resp.ResponseData, TotalRecords: resp.TotalRecords });
      } else {
        setState({ TestingTechniquesMaster: [], TotalRecords: 0 });
      }
    } catch (err: unknown) {
  ...
  const handleAddClient = (): void => {
    const CurrAddEditObj: TestingTechniqueItem = {
      TestingTechniqueId: '',
      TestingTechnique: '',
      Description: '',
    };
  ...
  const handleEdit = (item: TestingTechniqueItem): void => {
  ...
  const handleChangeClientInfo = (
    ...
    name: keyof TestingTechniqueItem
  ): void => {
    const CurrAddEditObj: TestingTechniqueItem = { ...state.CurrAddEditObj, [name]: e.target.value };
  ...
  const handleDeleteItem = async (item: TestingTechniqueItem): Promise<void> => {
    const resp = (await apiRequest('/DeleteTestingTechniquesMaster', item)) as BaseApiResponse | boolean | null;
    if (resp) {
  ...
  const handleSubmitClientInfo = async (): Promise<void> => {
    ...
    const resp = (await apiRequest('/AddUpdateTestingTechniquesMaster ', state.CurrAddEditObj)) as BaseApiResponse | null;
    if (resp) {
  ...
  const columns: ReadonlyArray<TableColumn> = [
    { title: '#ID', key: 'TestingTechniqueId', className: 'max-w-[30px]' },
    { title: 'Testing Technique', key: 'TestingTechnique' },
    { title: 'Description', key: 'Description' },
  ];

  const data: DataRow[] = state.TestingTechniquesMaster.map((v) => ({
    TestingTechniqueId: v.TestingTechniqueId,
    TestingTechnique: v.TestingTechnique,
        ...
        <div className=" w-full pt-2">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 gap-4">
            <div className="">
              ...
              <input
                onChange={(e): void => handleChangeClientInfo(e, 'TestingTechnique')}
                value={state.CurrAddEditObj.TestingTechnique}
                type="text"
                id="client"
                name="client"
                ...
                placeholder="Enter TestingTechnique"
                required
              />
              {state.FormErrors.TestingTechnique && (
                <div className="flex items-center mt-1 ml-2">
                  <CircleAlert size={14} className="text-red-500" />
                  <p className="ml-2 text-red-500 text-sm ">{state.FormErrors.TestingTechnique}</p>
                </div>
              )}
              {state.isDataExist && (
                <div className="flex items-center mt-1 ml-2">
