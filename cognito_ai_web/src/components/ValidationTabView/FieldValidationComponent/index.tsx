...
const FieldValidationComponent = forwardRef<any, FieldValidationProps>(
  (
    ...
    const primaryTableOptions = useMemo(
      () => tableData.map((t) => ({ 
        value: t.TableName, 
        label: t.TableDesc ? `${t.TableName} - ${t.TableDesc}` : t.TableName 
      })),
      [tableData],
    )
    ...
    const primaryKeyFieldOptions = useMemo(() => {
      ...
      return table.Fields.filter((f: FieldData) => f.KeyField === "Yes" && f.VerificationField === "Yes").map(
        (f: FieldData) => {
          ...
          const textValue = desc ? `${f.FieldName} - ${desc}` : f.FieldName
          // Create label with badges as React nodes for dropdown list
          const label = (
          ...
          return {
            value: f.FieldName,
            label,
            text: textValue, // Plain text for chip display
    ...
    const handleExecute = async () => {
      ...
      try {
        const sysNumResponse = await getSapSystemNumbersNew({
          payload: {
            data: { application: sourceApplication || "" },
    ...
    const handleModalSystemNumberChange = async (value: string) => {
      ...
      if (selectedSystemNumber) {
        setSystemNumberLabel(typeof selectedSystemNumber.label === 'string' ? selectedSystemNumber.label : String(selectedSystemNumber.value))
      }
    ...
    const handleModalClientIdChange = async (value: string) => {
      ...
      if (selectedClientId) {
        setClientIdLabel(typeof selectedClientId.label === 'string' ? selectedClientId.label : String(selectedClientId.value))
      }
            ...
            <div className="flex justify-end items-center pt-4">
              <div className="flex gap-2">
                <Button 
                  ...
                  disabled={isSaving}
                  className={!isCancelClicked ? "bg-white" : ""}
                >
                  Cancel
                </Button>
                {mode !== "view" && (  
                  <>
                    <Button 
                      ...
                      disabled={isSaving}
                      variant={isSaveClicked ? "default" : "outline"}
                      className={!isSaveClicked ? "bg-white" : ""}
                    >
                    ...
                    <Button 
                      ...
                      disabled={isExecutingValidation || !canExecuteValidation}
                      variant={isExecuteValidationClicked ? "default" : "outline"}
                      className={!isExecuteValidationClicked ? "bg-white " : ""}
                    >
        ...
        <Sheet open={isDrawerOpen} onOpenChange={handleDrawerClose}>
          ...
          >
            <FieldRulesEditor
              ...
              onSaveValidation={handleSaveClick}
              isSaving={isSaving}
              filterValues={filterValues}
              primary_table={primary_table}
              primary_key_fields={primary_key_fields}
